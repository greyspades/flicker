...
const SeriesGenres=(props)=>{
    ...
    useEffect(()=>{
            ...
            Axios.get(`https://api.themoviedb.org/3/genre/tv/list?api_key=99513a8369b9b5f2750aeee3e661a5ff&language=en-US`)
            .then((res)=>{
            if(!isCancelled){
                setloaded(true)
    ...
    <FlatGrid
             ...
             renderItem={({item})=>(
                <View>
                    <TouchableOpacity onPress={()=>{props.navigation.navigate('seriesmain',{item:item})}}>
...
export default connect(mapToProps,dispatchToProps)(SeriesGenres)